...
namespace MediaServer.Media.Services
{
    public class VideoMediaHandler : IMediaHandler
    {
        public async Task<Dictionary<string, string>> ExtractParametersAsync(MediaDescription media)
        {
            ...
            {
                parameters["Width"] = media.Width.Value.ToString();
            }

            if (media.Height.HasValue)
            {
                parameters["Height"] = media.Height.Value.ToString();
            }
        ...
        public async Task<string> ParseCodecAsync(string codecString)
        {
            ...
            // Örnek codecString: "96 H264/90000"
            ...
            if (parts.Length >= 2)
            {
                ...
                return await Task.FromResult(codecParts[0]); // "H264"
            }
        ...
        public async Task<Dictionary<string, string>> ParseFormatParametersAsync(string fmtpString)
        {
            ...
            // Örnek fmtpString: "96 packetization-mode=1; sprop-parameter-sets=Z0LACpZTBYmI,aMljiA=="
            var parameters = new Dictionary<string, string>();
        ...
        {
            return await Task.FromResult(new List<string> { "H264", "VP9", "AV1" });
        }
        ...
        {
            // İşleme mantığı burada olmalı
            // Bu örnekte sadece giriş akışını çıkış akışına kopyalıyoruz
        ...
        public async Task<CodecInfo> GetPreferredCodecAsync(MediaDescription media)
        {
            ...
            return new CodecInfo { Name = supportedCodecs.FirstOrDefault() ?? "H264" };
        }
    }
}